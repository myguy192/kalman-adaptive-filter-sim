...
SENSOR_STD_BASE = 0.40
SENSOR_STD_KV = 0.80

Q_BASE = 0.005

# NIS threshold for 2D Gaussian consistency checking
NIS_THRESH = 9.21
GATING_MODE = "inflate"
R_INFLATE_FACTOR = 50.0

# Pause-and-remeasure heuristic to reduce terminal error near goal
PAUSE_DIST = 0.3
PAUSE_P_AREA = 0.02
PAUSE_FRAMES = 10
...
def speed_schedule(dist):
    """
    Distance-based speed profile to avoid overshoot near the goal.
    """
    if dist <= STOP_RADIUS:
        return 0.0
    if dist >= SLOW_RADIUS:
...
def sensor_std_from_speed(vmag):
    # Models motion-induced degradation in position sensing
...
class OnlineRScaler:
    """
    Lightweight adaptive scaling model for measurement covariance.
    Trained online using simulation truth; intended for experimentation.
    """
...
def update(frame):
    ...
    if nis > NIS_THRESH:
        gated = True
        # Inflate R instead of skipping update to maintain filter continuity
        if GATING_MODE == "skip":
            x_hat = x_pred
            P = P_pred
        else:
